    ...
    <div class="container">
      ...
      <div class="row g-4 align-items-start profile-layout">
        ...
        <div class="col-lg-9 col-md-8">
          <div ref="contentScrollRef" class="profile-content-scroll">
              ...
              <div id="addresses" class="card border-0 shadow-sm profile-section">
                <div class="card-body p-4 p-lg-5">
                  <div class="d-flex align-items-center gap-2 mb-4">
                    ...
                    <a href="/address" class="text-decoration-none fw-semibold small">
                  ...
                  <form @submit.prevent="handleAddress" novalidate>
                    <div class="border rounded-4 p-3 p-md-4 address-box">
                      ...
                      <div class="row g-3">
                        ...
                        <div class="col-md-4">
                          <label class="form-label fw-semibold" for="postalCode"
                            >Codigo Postal</label
                          >
                          <input
                            id="postalCode"
...
const {
  ...
  activeSection,
  activeAction,
  scrollToCategory: viewModelScrollToCategory,
  runAction,
  updatePersonalData,
  updateAddress,
  changePassword,
...
const handleViewportChange = () => {
  isDesktopLayout.value = window.innerWidth >= 768;
...
const updateVisibleSection = () => {
  ...
  const anchorOffset = isDesktopLayout.value && container ? containerTop + 24 : 140;
  ...
  const offsets = navigationItems
    .map(({ id }) => {
      ...
      return {
        id,
        distance: Math.abs(element.getBoundingClientRect().top - anchorOffset),
      };
    })
...
const scrollToCategory = (categoryId: SectionId) => {
  ...
  if (!isDesktopLayout.value || !contentScrollRef.value) {
    viewModelScrollToCategory(categoryId);
    return;
  }
  ...
  container.scrollTo({
    top: Math.max(nextTop - 12, 0),
    behavior: "smooth",
  });
};

const handlePersonalData = async () => {
  await runAction("personal" as ActionKey, updatePersonalData);
};

const handleAddress = async () => {
  await runAction("address" as ActionKey, updateAddress);
};

const handlePassword = async () => {
  await runAction("password" as ActionKey, changePassword);
};

const handleDeleteAccount = async () => {
  await runAction("delete" as ActionKey, deleteAccount);
};

onMounted(async () => {
  handleViewportChange();
  await nextTick();
  contentScrollRef.value?.addEventListener("scroll", updateVisibleSection, { passive: true });
...
onBeforeUnmount(() => {
  contentScrollRef.value?.removeEventListener("scroll", updateVisibleSection);
